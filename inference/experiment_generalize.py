...
def _build_goal_and_rule_schedule(n_trials: int) -> tuple[list[str], list[str], int]:
    """Create aligned goal-star and rule schedules for the whole experiment.

    Returns:
        tuple: (goal_schedule, rule_schedule, transfer_start_trial)
    """
    if n_trials <= 0:
    ...
    transfer_start_trial = len(learning_goals)  # Transition point from learning to transfer
    return goal_schedule, rule_schedule, transfer_start_trial
...
def _run_encoding_group(
    ...
    for batch_num, batch_ids in enumerate(_chunked(participant_ids, n_agents), start=1):
        ...
        env = StarMakingEnvBatchText(
            ...
            rule_type="learning",
            notify_transfer=cfg.experiment.get("notify_transfer", False),
        )
        ...
        batch_results = env.run_experiment_batch(
            ...
            transfer_start_trial=transfer_start_trial,
        )
...
def main(cfg: DictConfig) -> None:
    ...
    goal_schedule, rule_schedule, transfer_start_trial = _build_goal_and_rule_schedule(n_trials)
    logger.info(
        "Trial split: %s learning / %s transfer (transition at trial %s)",
        rule_schedule.count("learning"),
        rule_schedule.count("transfer"),
        transfer_start_trial,
    )
    ...
    for ids, encoding in encoding_groups:
        all_agents.extend(_run_encoding_group(ids, encoding, cfg, llm_client, goal_schedule, rule_schedule, transfer_start_trial))
